class ProtoKD(nn.Module):
    def __init__(self,cfg):
        ...
        #model
        self.model = VNet(n_channels=cfg['student_channels'], n_classes=self.num_cls, n_filters=cfg['n_filters'],normalization='batchnorm')
        self.model.train()
        self.model.cuda()
        self.teacher_model = VNet(n_channels=cfg['teacher_channels'], n_classes=self.num_cls, n_filters=cfg['n_filters'],normalization='batchnorm')
        self.teacher_model.cuda()
        self.teacher_model.load_state_dict(torch.load(cfg['teachermodel_path']))
        self.teacher_model.eval()
        #optimizer
        self.optimizer = torch.optim.Adam(self.model.parameters(), lr=self.lr,weight_decay = cfg['weight_decay'])
        #dataloader
        # train_dataset = BraTS(cfg['data_dir'],crop_size=CROP_SIZE)
        # print('Training set includes %d data.' % len(train_dataset))
        # self.train_loader = DataLoader(dataset=train_dataset,batch_size=cfg['batch_size'], shuffle=True, num_workers = cfg['num_workers'], pin_memory=True)

        train_dataset = FloodsDataset('train',cfg['teacher_channels'],cfg['data_dir'])
        print('Training set includes %d data.' % len(train_dataset))
        ...
        val_dataset = FloodsDataset('val',cfg['teacher_channels'],cfg['data_dir'],min_ = train_dataset.x_min, max_ = train_dataset.x_max)
    ...
    def do_one_epoch(self,epoch):
        ...
        for idx, sampled_batch in enumerate(self.train_loader):
            ...
            #proto loss
            sim_map_s,sim_map_t,proto_loss = prototype_loss(feature,feature_t,label,self.num_cls)
            #loss
            ...
            # logging.info('Epoch:[%d/%d],iteration:%d, loss: %f' % (epoch,self.max_epoch,self.iter_num, loss.item()))
        time2 = time.time()
        epoch_loss = sum(loss_values)/len(loss_values)
        
        logging.info('Epoch:[%d/%d],iteration:%d, loss: %f' % (epoch, self.max_epoch, self.iter_num, epoch_loss))
        logging.info('Epoch %d training time :%f minutes' % (epoch, (time2-time1)/60))
        
    def validate(self,epoch):
        self.model.eval()
        dice_evaluator = Dice(num_classes= self.num_cls, average = 'macro').cuda()     
        ...
        with torch.no_grad():
            for idx, sampled_batch in enumerate(self.val_loader):
                image,label = sampled_batch
                image,label = image.float().cuda(), label.float().cuda()
                image = image[:,:2,:,:]

                _,predict = self.model(image)
        ...
        #         predict,_ = test_single_case(self.model,image,STRIDE,CROP_SIZE,self.num_cls)
...
def do_train(self):
        ...
        for epoch in range(self.start_epoch,self.max_epoch):
            curr_lr = self.lr * (1.0-np.float32(epoch)/np.float32(self.max_epoch))**(0.9)
            ...
            if epoch%25 == 0:
                ...
                torch.save(state,self.save_model_path+'/checkpoint_{}.pth.tar'.format(epoch))
        ...
        logging.info('Dice of wt/co/ec is %f,%f,%f'%(self.best_wt,self.best_co,self.best_ec))
